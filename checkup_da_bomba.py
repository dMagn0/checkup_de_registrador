...
def rotina_de_bomba(dict:dict) -> tuple[bool,str]:
    """
    ...
    if dict.get(FuncaoRegistrador.mt_tipo_motor).valor_de_leitura == 1:
        ...
        print(f"Valor lido do endereço 708 ({dict.get(FuncaoRegistrador.mt_tipo_motor).valor_de_leitura}) em contradicao com a tabela.")
        string_de_retorno+=" ? :\n"
...
def confere_controle(lista:list) -> tuple[bool,str]:
    """
    lista: lista com os registradores para teste de resposta

    return: true
    """

    lock = Lock()
    erro_de_resposta = False
    max_teste = 3
    string_de_retorno = "Controle de registrador da bancada:\n"

    for i in range(max_teste):
        with lock:
            for reg in lista:
                if len(reg.valores_de_escrita)>i:
                    reg.escrita(reg.valores_de_escrita[i])
        sleep(3)
        with lock:
            for reg in lista:
                reg.leitura()
                if len(reg.valores_de_escrita)>i:
                    if reg.valor_de_leitura != reg.valores_de_escrita[i]:
                        string_de_retorno+=f"    Sem resposta do registrador {reg.nome} ({reg.endereco}), valor (escrito: {reg.valores_de_escrita[i]}, lido: {reg.valor_de_leitura})\n"
                        erro_de_resposta = True
                        

    if erro_de_resposta:
        inp = int(input(f"Houve erro no teste de controle. Continuar? "))
        if inp == 0:
            string_de_retorno+="    Operacao cancelada."
            return (False,string_de_retorno)
    ...
    for l in lista:
        with lock:
            l.escrita(l.valores_de_escrita[0])
        pass
    
    string_de_retorno+="\n"
    return (True,string_de_retorno)

def coleta_dados_direta(dict_motor:dict, lista_controle:list,lista_leitura:list) -> dict:
    ...
    with lock:
        ...
        dict_motor.get(FuncaoRegistrador.mot_status_tesys).leitura()
    
    if dict_motor.get(FuncaoRegistrador.mt_habilita).valor_de_leitura != 1:
        with open('saida\\relatorio_bomba.txt','a') as f:
    ...
    lista_estados = bomba.get_estado_desligado()
    ...
    if dict_motor.get(FuncaoRegistrador.mot_indica_driver).valor_de_leitura != 3:
        with open('saida\\relatorio_bomba.txt','a') as f:
            f.write(f"        indica_driver com valor contraditorio. valor = [{dict_motor.get(FuncaoRegistrador.mot_indica_driver).valor_de_leitura}], esperado = [3]\n")
    if dict_motor.get(FuncaoRegistrador.mot_status_tesys).valor_de_leitura != 1:
        with open('saida\\relatorio_bomba.txt','a') as f:
            f.write(f"        status_tesys com valor contraditorio. valor = [{dict_motor.get(FuncaoRegistrador.mot_status_tesys).valor_de_leitura}], esperado = [1]\n")

    dicionario_de_amostras_motor_ligado = dict()
    lista_estados = bomba.get_estado_ligado()
    for i,l in enumerate(lista_estados):
...
def coleta_dados_soft(dict_motor:dict, lista_controle:list,lista_leitura:list) -> dict:
    NUMERO_DE_AMOSTRAS = 10
    TEMPO_ENTRE_AMOSTRA = 1
    lock = Lock()
...
def coleta_dados_inv(dict_motor:dict, lista_controle:list,lista_leitura:list) -> dict:
    NUMERO_DE_AMOSTRAS = 10
    TEMPO_ENTRE_AMOSTRA = 1
    lock = Lock()
...
def confere_reg_bancada(dict_motor:dict, dict_bancada:dict):
    ...
    with open('saida\\relatorio_bomba.txt','a') as f:
        f.write("\nRegistradores de medidas da bancada:\n")

    input("espere o reservatorio superior esvaziar para continuar o checkup.")
    input("feche a valvula superior.")

    with lock:
        dict_bancada["bc.na_tq_inferior"].leitura()
        dict_bancada["bc.nb_tq_inferior"].leitura()

    if dict_bancada["bc.na_tq_inferior"].valor_de_leitura != 1:
        with open('saida\\relatorio_bomba.txt','a') as f:
            f.write(f"    bc.na_tq_inferior com leitura fora do esperado: leitura [{dict_bancada["bc.na_tq_inferior"].valor_de_leitura}], esperado [1]\n")
    if dict_bancada["bc.nb_tq_inferior"].valor_de_leitura != 0:
        with open('saida\\relatorio_bomba.txt','a') as f:
            f.write(f"    bc.nb_tq_inferior com leitura fora do esperado: leitura [{dict_bancada["bc.nb_tq_inferior"].valor_de_leitura}], esperado [0]\n")
...
def escreve_dicionario(dicionario,estado=1):
    ...
    with open('saida\\bomba.txt','a') as f:
        for key, value in dicionario.items():
            if estado == 0:
                f.write(f"       {bomba.get_estado_desligado()[key]}:\n")
            else:
                f.write(f"       {bomba.get_estado_ligado()[key]}:\n")
            for key2,value2 in value.items():
                f.write(f"          {key2.nome} ({key2.descricao}):\n")
                f.write(f"              maximo: {max(value2)},  minimo: {min(value2)},  media: {sum(value2)/len(value2)},   variacao media por tempo: {variacao(value2)},   valores: {value2}")
...
if __name__=="__main__":
    ip = bomba.get_bomba_ip()
    isLocal = False
    if isLocal:
        ip = "localhost"
    comunicador = ComunicadorModbus(ip, bomba.get_bomba_port())
    lock = Lock()
    ...
    with open('saida\\bomba.txt','w') as f:
        f.write("")
    with open('saida\\relatorio_bomba.txt','w') as f:
        f.write("")
    
    #with open('saida\\bomba_relatorio.txt','a') as f:
    #    f.write("")

    #pega os registradores
    list_reg_saida = list()
    for i in bomba.get_reg_saida_motor():
        list_reg_saida.append(Registrador(comunicador,i)) 

    list_reg_controle = list()
    for i in bomba.get_reg_controle():
    ...
    while not leitura_verde or not leitura_azul:
        #confere motor
        resp, relatorio = rotina_de_bomba(dict_controlador_motor)
        with open('saida\\relatorio_bomba.txt','a') as f:
            f.write(relatorio)
        if resp == False:
            print("Checkup cancelado.")
        ...
        print("\nPartida Direta:")
        dict_direta = coleta_dados_direta(dict_controlador_motor, list_reg_controle, list_reg_saida)
        print("\nPartida Soft:")
        dict_soft = coleta_dados_soft(dict_controlador_motor, list_reg_controle, list_reg_saida)
        print("\nPartida Inversora:")
        dict_inversor = coleta_dados_inv(dict_controlador_motor, list_reg_controle, list_reg_saida)
        ...
        if dict_controlador_motor.get(FuncaoRegistrador.mt_tipo_motor).valor_de_leitura == 1:
            ...
            while dict_controlador_motor.get(FuncaoRegistrador.mt_tipo_motor).valor_de_leitura != 2 and leitura_azul == False:
                print("Favor ligar o motor azul da bomba para proseguir.\n")
                with lock:
                    dict_controlador_motor.get(FuncaoRegistrador.mt_tipo_motor).leitura()
                sleep(5)
                pass
            ...
            while dict_controlador_motor.get(FuncaoRegistrador.mt_tipo_motor).valor_de_leitura != 1 and leitura_verde == False:
                print("Favor ligar o motor verde da bomba para proseguir.\n")
                with lock:
                    dict_controlador_motor.get(FuncaoRegistrador.mt_tipo_motor).leitura()
                sleep(5)
                pass
    ...
    dict_reg_bancada = []
    for i in bomba.get_reg_saida_bancada():
        new_reg = Registrador(comunicador, i)
        dict_reg_bancada[new_reg.nome] = new_reg

    confere_reg_bancada(dict_controlador_motor,dict_reg_bancada)
