...
def rotina_de_motor(dict:dict) -> tuple[bool,str]:
    """
    ...
    while dict.get(FuncaoRegistrador.mt_tipo_motor).valor_de_leitura == 0:
        print("Favor ligar o motor do ventilador antes de comecar.\n")
        sleep(5)
        with lock:
            ...
            for reg in lista:
                if len(reg.valores_de_escrita)>i:
                    if reg.valor_de_leitura != reg.valores_de_escrita[i]:
                        print(f"Sem resposta do registrador {reg.nome} ({reg.endereco}), valor (escrito: {reg.valores_de_escrita[i]}, lido: {reg.valor_de_leitura})\n")
...
def coleta_dados_direta(dict_motor:dict, lista_controle:list,lista_leitura:list) -> dict:
    ...
    if dict_motor.get(FuncaoRegistrador.mt_habilita).valor_de_leitura != 1:
        with open('saida\\relatorio_ventilador.txt','a') as f:
    ...
    lista_estados = ventilador.get_estado_desligado()
    ...
    lista_estados = ventilador.get_estado_ligado()
    for i,l in enumerate(lista_estados):
...
def escreve_dicionario(dicionario,estado=1):
    ...
    with open('saida\\ventilador.txt','a') as f:
        for key, value in dicionario.items():
            if estado == 0:
                f.write(f"       {ventilador.get_estado_desligado()[key]}:\n")
            else:
                f.write(f"       {ventilador.get_estado_ligado()[key]}:\n")
...
if __name__=="__main__":
    ip = ventilador.get_ventilador_ip()
    isLocal = False
    if isLocal:
        ip = "localhost"

    comunicador = ComunicadorModbus(ventilador.get_ventilador_ip(), ventilador.get_ventilador_port())
    lock = Lock()
    ...
    for i in ventilador.get_reg_motor():
    ...
    with open('saida\\ventilador.txt','w') as f:
        f.write("")
    with open('saida\\relatorio_ventilador.txt','w') as f:
        f.write("")

    #pega os registradores
    list_reg_saida = list()
    for i in ventilador.get_reg_saida_motor():
        list_reg_saida.append(Registrador(comunicador,i)) 

    list_reg_controle = list()
    """
    for i in ventilador.get_reg_controle():
        list_reg_controle.append(Registrador(comunicador,i)) 
    
    #confere reg de controle  
    if confere_controle(list_reg_controle) == False:
    ...
    while not leitura_verde or not leitura_azul:
        ...
        if dict_controlador_motor.get(FuncaoRegistrador.mt_tipo_motor).valor_de_leitura == 1:
            ...
            while dict_controlador_motor.get(FuncaoRegistrador.mt_tipo_motor).valor_de_leitura != 2 and leitura_azul == False:
                print("Favor ligar o motor azul do ventilador para proseguir.\n")
                with lock:
                    dict_controlador_motor.get(FuncaoRegistrador.mt_tipo_motor).leitura()
                sleep(5)
                pass
            ...
            while dict_controlador_motor.get(FuncaoRegistrador.mt_tipo_motor).valor_de_leitura != 1 and leitura_verde == False:
                print("Favor ligar o motor verde do ventilador para proseguir.\n")
                with lock:
                    dict_controlador_motor.get(FuncaoRegistrador.mt_tipo_motor).leitura()
                sleep(5)
                pass
